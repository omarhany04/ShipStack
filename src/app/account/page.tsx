...
export default function AccountPage() {
  ...
  if (isLoading) {
    return (
      <div className="mx-auto flex min-h-[60vh] max-w-7xl items-center justify-center px-4 py-10 sm:px-6 lg:px-8">
  ...
  if (!profile) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-16 text-center sm:px-6 lg:px-8">
        <p className="text-lg font-semibold text-slate-900">We couldn&apos;t load your account.</p>
        <p className="mt-3 text-sm text-slate-500">{profileError ?? 'Please refresh and try again.'}</p>
        <Link
          href="/"
            ...
            <div className="flex-1 space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(event) => {
                    setProfile((current) => (current ? { ...current, company: event.target.value } : current));
                    setProfileMessage(null);
                  }}
